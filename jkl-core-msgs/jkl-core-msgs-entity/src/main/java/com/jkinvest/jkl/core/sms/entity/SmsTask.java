package com.jkinvest.jkl.core.sms.entity;
...
/**
 * <p>
 * 实体类 发送任务 所有的短息发送调用，都视为是一次短信任务，任务表只保存数据和执行状态等信息， 具体的发送状态查看发送状态（#sms_send_status）表
 * </p>
 *
 * @author zuihou
 * @since 2019-11-22
...
public class SmsTask extends Entity<Long> {
  ...
  /**
   * 执行状态 (手机号具体发送状态看sms_send_status表) #TaskStatus{WAITING:等待执行;SUCCESS:执行成功;FAIL:执行失败}
   */
  ...
  /**
   * 来源类型 #SourceType{APP:应用;SERVICE:服务}
   */
  ...
  /**
   * 接收者手机号 群发用英文逗号分割. 支持2种格式: 1: 手机号,手机号 2: 姓名<手机号>,姓名<手机号>
   */
  ...
  /**
   * 参数 需要封装为{‘key’:’value’, ...}格式 且key必须有序
   */
  ...
  @Builder
  public SmsTask(Long id, Long createUser, LocalDateTime createTime, Long updateUser,
      LocalDateTime updateTime, Long templateId, TaskStatus status, SourceType sourceType,
      String receiver, String topic, String templateParams, LocalDateTime sendTime, String content,
      Boolean draft) {
    this.id = id;
